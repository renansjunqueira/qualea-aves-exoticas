'use client'
import Link from 'next/link'
import Image from 'next/image'
import { usePathname } from 'next/navigation'
import { cn } from '@/lib/utils'
import {
  LayoutDashboard, Bird, Heart, Egg, BookOpen, Settings, LogOut, Users,
} from 'lucide-react'
import { signOut } from '@/lib/actions/auth'

const NAV = [
  { href: '/',          label: 'Dashboard',  icon: LayoutDashboard },
  { href: '/birds',     label: 'Plantel',    icon: Bird },
  { href: '/pairs',     label: 'Casais',     icon: Heart },
  { href: '/clutches',  label: 'Ninhadas',   icon: Egg },
  { href: '/species',   label: 'Espécies',   icon: BookOpen },
]

interface Props {
  userEmail?: string
  isAdmin?:   boolean
}

export function Sidebar({ userEmail, isAdmin }: Props) {
  const pathname = usePathname()

  return (
    <aside className="hidden lg:flex flex-col fixed inset-y-0 left-0 w-60 z-40" style={{ background: 'var(--color-primary-800)' }}>
      {/* Logo */}
      <div className="flex items-center gap-3 px-4 py-3 bg-white border-b" style={{ borderColor: 'rgba(29,45,68,0.08)' }}>
        <div className="relative w-12 h-12 flex-shrink-0">
          <Image src="/logo-misto3.png" alt="Qualea Tech" fill className="object-contain" />
        </div>
        <span
          className="font-semibold tracking-tight leading-none"
          style={{ fontSize: '17px', color: '#1D2D44', letterSpacing: '-0.02em' }}
        >
          Qualea<span style={{ color: 'var(--color-primary-500)' }}> Tech</span>
        </span>
      </div>

      {/* Nav */}
      <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto">
        <p className="text-[10px] font-semibold uppercase tracking-widest px-3 pb-2" style={{ color: 'var(--color-primary-400)' }}>
          Menu
        </p>
        {NAV.map(({ href, label, icon: Icon }) => {
          const active = pathname === href || (href !== '/' && pathname.startsWith(href))
          return (
            <Link key={href} href={href} className={cn('nav-link', active && 'active')}>
              <Icon size={17} strokeWidth={active ? 2.2 : 1.8} />
              {label}
            </Link>
          )
        })}

        {/* Links de administração (apenas para admins) */}
        {isAdmin && (
          <>
            <p className="text-[10px] font-semibold uppercase tracking-widest px-3 pb-2 pt-4" style={{ color: 'var(--color-primary-400)' }}>
              Administração
            </p>
            <Link
              href="/admin/users"
              className={cn('nav-link', pathname.startsWith('/admin/users') && 'active')}
            >
              <Users size={17} strokeWidth={pathname.startsWith('/admin/users') ? 2.2 : 1.8} />
              Usuários
            </Link>
          </>
        )}
      </nav>

      {/* Footer */}
      <div className="px-3 py-4 border-t" style={{ borderColor: 'rgba(255,255,255,0.08)' }}>
        <Link href="/settings" className="nav-link">
          <Settings size={17} strokeWidth={1.8} />
          Configurações
        </Link>

        {/* Usuário + logout */}
        <div className="mt-3 px-3">
          {userEmail && (
            <p className="text-[11px] truncate mb-2" style={{ color: 'var(--color-primary-400)' }}>
              {userEmail}
            </p>
          )}
          <form action={signOut}>
            <button
              type="submit"
              className="flex items-center gap-2 text-[13px] font-medium transition-colors hover:text-white"
              style={{ color: 'var(--color-primary-400)' }}
            >
              <LogOut size={14} />
              Sair
            </button>
          </form>
        </div>
      </div>
    </aside>
  )
}
